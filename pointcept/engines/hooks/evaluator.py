...
@HOOKS.register_module()
class MultiHeadSemSegEvaluator(HookBase):
    def __init__(self):
        pass
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                ...
                if isinstance(input_dict[key], list):
                    for idx in range(len(input_dict[key])):
                        if isinstance(input_dict[key][idx], torch.Tensor):
                            input_dict[key][idx] = input_dict[key][idx].cuda(
                                non_blocking=True
                            )
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict)
            outputs = output_dict["seg_logits"]
            loss = output_dict["loss"]
            preds = torch.stack(
                [out.max(1)[1] for out in outputs], dim=-1
            )  # (n, num_heads)
            segments = input_dict["segments"]
            # assert "inverse" not in input_dict.keys()

            for j, head_sz in enumerate(self.trainer.cfg.data.heads):
                intersection, union, target = intersection_and_union_gpu(
                    preds[:, j],
                    segments[j],
                    head_sz,
                ...
                self.trainer.storage.put_scalar(
                    f"val_intersection_layer_{j}", intersection
                )
                self.trainer.storage.put_scalar(f"val_union_layer_{j}", union)
                self.trainer.storage.put_scalar(f"val_target_layer_{j}", target)

            self.trainer.storage.put_scalar(f"val_loss", loss.item())

            # assert "origin_coord" not in input_dict.keys()
            self.trainer.logger.info(
        ...
        for i, head_sz in enumerate(self.trainer.cfg.data.heads):
            intersection = self.trainer.storage.history(
                f"val_intersection_layer_{i}"
            ).total
            union = self.trainer.storage.history(f"val_union_layer_{i}").total
            target = self.trainer.storage.history(f"val_target_layer_{i}").total
            iou_class = intersection / (union + 1e-10)
            acc_class = intersection / (target + 1e-10)
            m_iou = np.mean(iou_class)
            m_acc = np.mean(acc_class)
        ...
        for i, head_sz in enumerate(self.trainer.cfg.data.heads):
            self.trainer.logger.info(
                "Layer {} ({}) Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                    i,
                    self.trainer.cfg.data.heads_names[i],
                    layers_metrics[i]["m_iou"],
                    layers_metrics[i]["m_acc"],
                    layers_metrics[i]["all_acc"],
                )
            )
            for j in range(head_sz):
                self.trainer.logger.info(
                    "Layer {} Class_{idx} - '{name}' Result: iou/accuracy {iou:.4f}/{accuracy:.4f}".format(
                        i,
                        idx=j,
                        name=self.trainer.cfg.data.heads_class_names[i][j],
                        iou=layers_metrics[i]["iou_class"][j],
                        accuracy=layers_metrics[i]["acc_class"][j],
                    )
                )

        # Average metric across all layers...
        m_iou = np.mean([m["m_iou"] for m in layers_metrics])
        m_acc = np.mean([m["m_acc"] for m in layers_metrics])
        all_acc = np.mean([m["all_acc"] for m in layers_metrics])
        self.trainer.logger.info(
            f"Average results across all layers: mIoU/mAcc/allAcc "
            f"{m_iou:.4f}/{m_acc:.4f}/{all_acc:.4f}"
        )
        ...
        if self.trainer.cfg.enable_wandb:
            ...
            for i in range(len(self.trainer.cfg.data.heads)):
                log_dict[f"val/mIoU_layer_{i}"] = layers_metrics[i]["m_iou"]
                log_dict[f"val/mAcc_layer_{i}"] = layers_metrics[i]["m_acc"]
                log_dict[f"val/allAcc_layer_{i}"] = layers_metrics[i]["all_acc"]
            wandb.log(log_dict, step=wandb.run.step)

        # save to trainer.writer
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/mIoU", m_iou, current_epoch)
            self.trainer.writer.add_scalar("val/mAcc", m_acc, current_epoch)
        ...
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = m_iou  # save for saver
        self.trainer.comm_info["current_metric_name"] = "layers_mIoU"  # save for saver
...
class SemSegEvaluatorTrain(HookBase):
    ...
    def eval(self):
        ...
        if self.trainer.writer is not None:
            ...
            wandb.log({"train/mIoU": m_iou, "train/mAcc": m_acc}, step=wandb.run.step)
            if self.write_cls_iou:
                for i in range(self.trainer.cfg.data.num_classes):
                    self.trainer.writer.add_scalar(
