...
@TESTERS.register_module()
class MultiHeadSemSegTester(TesterBase):
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")

        # metrics are per-layer
        batch_time = AverageMeter()
        self.model.eval()
        ...
        for idx, data_dict in enumerate(self.test_loader):
            start = time.time()
            data_dict = data_dict[0]  # current assume batch size is 1
            # assert "origin_segment" not in data_dict.keys()
            fragment_list = data_dict.pop("fragment_list")
            segments = data_dict.pop("segments")
            data_name = data_dict.pop("name")
            pred_save_path = os.path.join(save_path, "{}_pred.npy".format(data_name))
            if os.path.isfile(pred_save_path):
                logger.info(
                ...
                preds = np.load(pred_save_path)
            else:
                preds = [
                    torch.zeros((segments[0].size, h)).cuda()
                    for h in self.cfg.data.heads
                ]
                for i in range(len(fragment_list)):
                    fragment_batch_size = 1
                    s_i, e_i = i * fragment_batch_size, min(
                    ...
                    with torch.no_grad():
                        ...
                        pred_part = [
                            F.softmax(pred_part[j], -1)
                            for j in range(len(self.cfg.data.heads))
                        ]
                        if self.cfg.empty_cache:
                            torch.cuda.empty_cache()
                        bs = 0
                        for be in input_dict["offset"]:
                            for j in range(len(self.cfg.data.heads)):
                                preds[j][idx_part[bs:be], :] += pred_part[j][bs:be]
                            bs = be
                ...
                preds = [
                    preds[j].max(1)[1].data.cpu().numpy()
                    for j in range(len(self.cfg.data.heads))
                ]  # [(n,), (n,) ...]
                preds = np.stack(preds, axis=-1)  # (n, num_heads)
                np.save(pred_save_path, preds)

            records = []
            accs = []
            ious = []
            for i, head_sz in enumerate(self.cfg.data.heads):
                intersection, union, target = intersection_and_union(
                    preds[:, i], segments[i], head_sz
                )
                records.append(
                    dict(intersection=intersection, union=union, target=target)
                )
            ...
            record[data_name] = records
            batch_time.update(time.time() - start)
            logger.info(
                "Test: {} [{}/{}]-{} "
                "Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) "
                "Average accuracy {avgAcc:.4f} "
                "Average IoU {avgIoU:.4f}".format(
                    data_name,
                    idx + 1,
                    len(self.test_loader),
                    segments[0].size,
        ...
        for i, head_sz in enumerate(self.cfg.data.heads):
            intersection = np.sum(
                [meters[i]["intersection"] for _, meters in record.items()], axis=0
            )
            union = np.sum([meters[i]["union"] for _, meters in record.items()], axis=0)
            target = np.sum(
                [meters[i]["target"] for _, meters in record.items()], axis=0
            )
            ...
            logger.info(
                "Layer {} ({}) Val results: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}".format(
                    i, self.cfg.data.heads_names[i], mIoU, mAcc, allAcc
                )
            )

            for j in range(head_sz):
                logger.info(
                    "Layer {} Class_{idx} ({name}) "
                    "Result: iou/accuracy {iou:.4f}/{accuracy:.4f}".format(
                        i,
                        idx=j,
                        name=self.cfg.data.heads_class_names[i][j],
                        iou=iou_class[j],
                        accuracy=accuracy_class[j],
                    )
                )

        logger.info(
            f"Average results across all layers: mIoU/mAcc/allAcc "
            f"{np.mean(mIoUs):.4f}/{np.mean(mAccs):.4f}/{np.mean(allAccs):.4f}"
        )
        logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
