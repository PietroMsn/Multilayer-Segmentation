...
@MODELS.register_module()
class MultiHeadDefaultSegmentor(nn.Module):
    def __init__(
        self,
        heads,
        backbone_embed_dim=-1,
        backbone=None,
        criteria=None,
    ):
        super().__init__()
        ...
        for h in heads:
            if isinstance(h, int):
                self.heads.append(
                    nn.Sequential(
                        nn.Linear(backbone_embed_dim, 64),
                        nn.BatchNorm1d(64),
                        nn.ReLU(),
                        nn.Linear(64, 64),
                        nn.BatchNorm1d(64),
                ...
                self.heads.append(build_model(h))
    ...
    def forward(self, input_dict):
        ...
        # train
        if self.training:
            loss = self.criteria(logits, input_dict["segments"])
...
@MODELS.register_module()
class DefaultSegmentationHead(nn.Module):
    def __init__(
        self,
        backbone_embed_dim,
        layers,
        ...
        self.cls_head.extend(
            (nn.Dropout(p=dropout), nn.Linear(output_dim, num_classes))
        )
